  ...
  return <div className="workbench">
    ...
    <div className="workbench-grid">
      ...
      <section className="analysis-panel">
        ...
        {application.adversarial && <DevilsAdvocateReport adversarial={application.adversarial} />}
        {application.validator_report && <ValidatorReport report={application.validator_report} brief={application.decision_brief} />}
...
function DevilsAdvocateReport({ adversarial }: { adversarial: NonNullable<Application["adversarial"]> }) { return <section className="analysis-section adversary-section"><div className="section-title"><Bot size={18} /><div><h3>Devil&apos;s Advocate Report</h3><p>{adversarial.persona} · strongest bounded counter-case</p></div></div>{adversarial.objections.map((objection, index) => <article className="objection" key={`${objection.text}-${index}`}><span className={`verification ${objection.label === "speculation" ? "n/a" : "unverified"}`}>{objection.label}</span><p>{objection.text}</p><small>Targets: {objection.targets.length ? objection.targets.join(", ") : "none"} · {objection.evidence?.length || 0} cited Memory signals</small></article>)}</section>; }

function ValidatorReport({ report, brief }: { report: NonNullable<Application["validator_report"]>; brief: Application["decision_brief"] }) { return <section className="analysis-section validator-section"><div className="section-title"><ShieldAlert size={18} /><div><h3>Validator Report</h3><p>Independent evidence check of every counter-case objection</p></div></div><div className="validator-list">{report.findings.map((finding) => <article className="validator-finding" key={finding.objection_i}><span className={`verification ${finding.status}`}>{finding.status}</span><div><p>{finding.explanation}</p><small>Targets: {finding.targets.length ? finding.targets.join(", ") : "none"} · {finding.evidence.length} validated evidence records</small></div></article>)}</div><div className="validator-summary"><b>Validator summary</b><p>{report.summary}</p></div>{brief && <div className="decision-brief"><ClipboardCheck size={17} /><div><b>Decision Brief</b><p>{brief.summary}</p></div></div>}</section>; }
